...
class TestReciprocalLatticeVector:
    ...
    def test_gspacing_dspacing_scattering_parameter(self, ferrite_phase):
        ...
        assert np.allclose(
            rlv.gspacing,
            np.array([
                0.34885749, 0.69771498, 0.493359, 0.78006907, 0.98671799, 0.78006907,
                0.604238, 0.85452285, 1.04657248, 1.2084778
            ])
        )
        assert np.allclose(
            rlv.dspacing,
            np.array([
                2.8665, 1.43325, 2.02692159, 1.28193777, 1.01346079, 1.28193777,
                1.65497455, 1.17024372, 0.9555, 0.8274872
            ])
        )
        assert np.allclose(
            ...
            np.array([
                ...
                0.39003453, 0.30211945, 0.42726142, 0.52328624, 0.6042389
            ])
        )
        # fmt: on
